...
public class checkc extends javax.swing.JFrame {
    ...
    private void showdatatable(int list){
        ...
        if (list == 1) {
            sql = "SELECT `chemical`.`ID`, `chemical`.`Chemical`, `chemical`.`Chemical_fomula` AS Formular,"
                    + "`chemical`.`Expire_date` AS Expire,`status`.`status` AS Status FROM `chemical`,`status` "
                    + "WHERE `chemical`.`status` = `status`.`id` ORDER BY `Chemical` ASC";
        } else if (list == 2) {
            sql = "SELECT `chemical`.`ID`, `chemical`.`Chemical`, `chemical`.`Chemical_fomula` AS Formular ,"
                    + "`chemical`.`Expire_date` AS Expire ,`status`.`status` AS Status FROM `chemical`,`status` "
                    + "WHERE `chemical`.`status` = `status`.`id` ORDER BY `chemical`.`ID` ASC";
        }
        try {
    ...
    private void initComponents() {
        ...
        sorttable.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "A-Z", "ID" }));
    ...
    private void searchInputKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchInputKeyReleased
        DefaultTableModel table = (DefaultTableModel)showtable.getModel();
        String search = searchInput.getText();
        TableRowSorter sorter = new TableRowSorter(showtable.getModel());
